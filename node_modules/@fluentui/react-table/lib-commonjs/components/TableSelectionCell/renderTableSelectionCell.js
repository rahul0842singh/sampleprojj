"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.renderTableSelectionCell_unstable = void 0;
const React = /*#__PURE__*/require("react");
const react_utilities_1 = /*#__PURE__*/require("@fluentui/react-utilities");
/**
 * Render the final JSX of TableSelectionCell
 */
const renderTableSelectionCell_unstable = state => {
  const {
    slots,
    slotProps
  } = react_utilities_1.getSlots(state);
  return React.createElement(slots.root, {
    ...slotProps.root
  }, state.type === 'checkbox' && slots.checkboxIndicator && React.createElement(slots.checkboxIndicator, {
    ...slotProps.checkboxIndicator
  }), state.type === 'radio' && slots.radioIndicator && React.createElement(slots.radioIndicator, {
    ...slotProps.radioIndicator
  }));
};
exports.renderTableSelectionCell_unstable = renderTableSelectionCell_unstable;
//# sourceMappingURL=renderTableSelectionCell.js.map